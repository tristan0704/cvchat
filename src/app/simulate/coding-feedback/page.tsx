...
function CodingFeedbackPageContent() {
    ...
    const nextHref = `/simulate/analysis?${new URLSearchParams({ role, outcome: "offer" }).toString()}`
        ...
        <main className="min-h-screen bg-slate-50 text-slate-900">
            <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
                <header className="mb-6 flex items-center justify-between rounded-[28px] border bg-white p-4">
                    ...
                    <Link href={`/simulate/coding?${new URLSearchParams({ role }).toString()}`} className="rounded-full border px-4 py-2 text-sm">
                        Zurueck
                    </Link>
                </header>
                ...
                <section className="rounded-[28px] border bg-white p-6 sm:p-8">
                    ...
                    <h2 className="mt-3 text-3xl font-semibold tracking-tight">Coding Feedback</h2>

                    <div className="mt-6 space-y-3">
                        <div className="rounded-2xl border px-4 py-3 text-sm text-slate-800">Problemverstaendnis wird spaeter bewertet.</div>
                        <div className="rounded-2xl border px-4 py-3 text-sm text-slate-800">Code-Qualitaet folgt spaeter.</div>
                        <div className="rounded-2xl border px-4 py-3 text-sm text-slate-800">Das Ergebnis fliesst spaeter in die Gesamtanalyse.</div>
                    </div>
